...
**Tareas:** tools `comparar_instrumentos`, `detalle_institucion`, `calcular_rendimiento_real`, `frescura_datos`; instrucciones del servidor con el disclaimer (no es asesoría); servicio `mcp` en el compose de Centinela (puerto propio en loopback, p. ej. 8012 — el 8001 lo ocupa el MCP de NarrativeAlpha) más una ruta `handle /mcp*` en el site block compartido de `centinelafinanciero.lat`; auth por bearer token.
